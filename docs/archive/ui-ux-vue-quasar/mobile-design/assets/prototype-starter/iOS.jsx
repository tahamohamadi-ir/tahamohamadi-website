  ...
  return (
    ...
    >
      ...
      <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
        {title !== undefined ? <IOSNavBar title={title} dark={dark} /> : null}
        <div style={{ flex: 1, overflow: "auto" }}>{children}</div>
        {keyboard ? <IOSKeyboard dark={dark} /> : null}
      </div>
      <div
        style={{
          position: "absolute",
...
function IOSKeyboard({ dark = false }) {
  ...
  const row = (keys, pad = 0) => (
    <div style={{ display: "flex", gap: 6.5, justifyContent: "center", padding: `0 ${pad}px` }}>
      {keys.map((value) => key(value, { flex: true, k: value }))}
    </div>
  );
      ...
      >
        {['"The"', "the", "to"].map((word, index) => (
          <React.Fragment key={index}>
            {index > 0 ? <div style={{ width: 1, height: 25, background: "#ccc", opacity: 0.3 }} /> : null}
            <div
              style={{
                flex: 1,
                textAlign: "center",
