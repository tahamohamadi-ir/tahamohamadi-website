...
- [ ] visual snapshotها، full frontend regression، production build و live feed review در این برش fast-track اجرا نشدند. **Owner:** release. **Trigger:** acceptance نهایی Phase 3. **Mitigation:** فقط 4 فایل متمرکز Vitest با 30 test اجرا شدند؛ conversion lossless نبودن را gate می‌کند، preview فقط media فعال را می‌پذیرد و هیچ قابلیت embed/raw HTML افزوده نشد.
